...
export default function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (path === "/properties/featured" && req.method === "GET") {
    const featured = mockProperties.filter((p) => p.isPopular || p.isNew).slice(0, 4);
    return res.json(featured);
  }

  // GET /api/properties/:id
  const propertyIdMatch = path.match(/^\/properties\/(\w+)$/);
  if (propertyIdMatch && req.method === "GET") {
    const property = mockProperties.find((p) => p.id === propertyIdMatch[1]);
    if (!property) {
  ...
  // GET /api/properties
  if ((path === "/properties" || path === "/properties/") && req.method === "GET") {
    try {
